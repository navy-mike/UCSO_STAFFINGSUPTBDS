...
type sle_empname from singlelineedit within w_insert_position
end type
type st_11 from statictext within w_insert_position
end type
...
ls_ua_sql += "and a.grpid = " + string(ll_grpid) + " "
ls_ua_sql += "and instr(a.empname, 'UNFILLED') > 0 "
...
if(f_len_ext(ls_callnum) <= 2) then
	...
	//get next available record to make unassigned
	long ll_next_id
	ll_next_id = 0
	//
	select id into :ll_next_id from sns_board_specs_others where grpid = :ll_grpid and active = 1 and empname is null and empno = 0 and callnum is null
	and positionid is null order by nrow, ncol fetch first row only using sqlca;
	//
	if(ll_next_id > 0) then
		//
		update sns_board_specs_others set empname = '*** UNFILLED ***', callnum = :ls_callnum, empno = 0, orderby = 0 where id = :ll_next_id using sqlca;
		//
		if(sqlca.sqlcode <> -1) then
			...
			//
			bSaved = true
			CloseWithReturn(w_insert_position, "SUCCESS")
		else
			ls_sql_err_text = sqlca.sqlerrtext
			//
			rollback using sqlca;
			//
			MessageBox("Notification", "Failed to set UNFILLED record. error=" + ls_sql_err_text)
		end if
	else
		MessageBox("Notification","No available record in table.")	
	end if
