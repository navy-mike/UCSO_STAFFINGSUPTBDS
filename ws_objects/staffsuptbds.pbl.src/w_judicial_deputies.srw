$PBExportHeader$w_judicial_deputies.srw
forward
global type w_judicial_deputies from window
end type
type dw_jud_group6 from datawindow within w_judicial_deputies
end type
type dw_jud_group5 from datawindow within w_judicial_deputies
end type
type dw_jud_group4 from datawindow within w_judicial_deputies
end type
type dw_jud_group3 from datawindow within w_judicial_deputies
end type
type dw_jud_supr2 from datawindow within w_judicial_deputies
end type
type st_4 from statictext within w_judicial_deputies
end type
type st_2 from statictext within w_judicial_deputies
end type
type st_3 from statictext within w_judicial_deputies
end type
type cbx_show_birthdays from checkbox within w_judicial_deputies
end type
type dw_jud_chief from datawindow within w_judicial_deputies
end type
type pb_1 from picturebutton within w_judicial_deputies
end type
type pb_2 from picturebutton within w_judicial_deputies
end type
type st_1 from statictext within w_judicial_deputies
end type
end forward
...
global w_judicial_deputies w_judicial_deputies

on w_judicial_deputies.create
this.dw_jud_group6=create dw_jud_group6
this.dw_jud_group5=create dw_jud_group5
this.dw_jud_group4=create dw_jud_group4
this.dw_jud_group3=create dw_jud_group3
this.dw_jud_supr2=create dw_jud_supr2
...
this.Control[]={this.dw_jud_group6,&
this.dw_jud_group5,&
this.dw_jud_group4,&
this.dw_jud_group3,&
this.dw_jud_supr2,&
...
on w_judicial_deputies.destroy
destroy(this.dw_jud_group6)
destroy(this.dw_jud_group5)
destroy(this.dw_jud_group4)
destroy(this.dw_jud_group3)
destroy(this.dw_jud_supr2)
destroy(this.st_4)
destroy(this.st_2)
destroy(this.st_3)
...
string title = "none"
string dataobject = "dw_jud_groups_double"
boolean livescroll = true
borderstyle borderstyle = styleshadowbox!
end type

event retrieveend;long ll_clicked_grpid
string ls_subgrpid
ll_clicked_grpid = 140
ls_subgrpid = "G6"
////=======================
...
string ls_empno
string ls_grpteam
dw_clicked_object = f_getDwDispJudObjectByGrpid_mod(ll_clicked_grpid, ls_subgrpid, dw_jud_chief, dw_null, dw_jud_supr2, dw_null, dw_null, dw_jud_group3, dw_jud_group4, dw_jud_group5, dw_jud_group6, dw_null, ref li_valid_object)
if(li_valid_Object = 1) then
	if(gb_show_all_birthdays = true) then
		integer li_num_objects
		li_num_objects = dw_clicked_object.getitemnumber(1, "sns_board_specs_numobjects")
		if(li_num_objects > 0) then
			for ll_loop = 1 to li_num_objects
				ls_rowid = "_" + f_lpad(string(ll_loop), 3, "0")
				ls_dispbd = "dispbd" + ls_rowid
				try
					li_dispbd = dw_clicked_object.getitemnumber(1, ls_dispbd)
					if(IsNull(li_dispbd) = true) then
						exit
					else
						if(li_dispbd = 0) then
							li_oktoshowbd = 0
							ls_empno = "empno" + ls_rowid;
							ll_empno = dw_clicked_object.getitemnumber(1, ls_empno)
							//
							select oktoshowbd into :li_oktoshowbd from sns_employees where active = 1 and empno = :ll_empno using sqlca;
							//
							if(li_oktoshowbd = 1) then
								dw_clicked_object.setitem(1, ls_dispbd, 4)
							end if
						end if
					end if
...
//	dw_clicked_object.Modify("DataWindow.Color='13011456'")
//end if
//
///=======================
long ll_clicked_grpid
...
event clicked;close(w_judicial_deputies)
end event
