$PBExportHeader$w_add_lt.srw
forward
global type w_add_lt from window
end type
type sle_grpid from singlelineedit within w_add_lt
end type
type sle_filtered_count from singlelineedit within w_add_lt
end type
type sle_selected_employee from singlelineedit within w_add_lt
end type
type sle_lastname_filter from singlelineedit within w_add_lt
end type
type st_2 from statictext within w_add_lt
end type
type pb_close from picturebutton within w_add_lt
end type
type dw_add_lt_list from datawindow within w_add_lt
end type
type sle_lookup_lastusername from singlelineedit within w_add_lt
end type
type st_1 from statictext within w_add_lt
end type
end forward
...
st_2 st_2
pb_close pb_close
dw_add_lt_list dw_add_lt_list
sle_lookup_lastusername sle_lookup_lastusername
st_1 st_1
end type
global w_add_lt w_add_lt
...
this.dw_add_lt_list=create dw_add_lt_list
...
if(f_len_ext(ls_messageparm) > 0) then
	...
	dw_add_lt_list.settransobject(sqlca)
	dw_add_lt_list.retrieve()
else
	CloseWithReturn(w_add_lt, "")
end if
...
string text = "~'Double click on username to use~'"
...
integer height = 608
integer taborder = 20
string dataobject = "dw_lieutenants_list_edit"
...
if(ls_dwo_name = 'COMPUTE_0001') then
	...
	string ls_empno
	long ll_empno
	ls_empname = dw_add_lt_list.getitemstring(row, 'COMPUTE_0001') //THOMAS-PHILIP       {100-42110}
	if(len(ls_empname) > 0) then
		...
		if(len(ls_empname) > 0) then
			...
			if(len(ls_lastname) > 0) then
				CloseWithReturn(w_add_lt, ls_empname)
			end if
	   end if
	end if
end if
...
dw_add_lt_list.selectrow(0, false)
dw_add_lt_list.visible = true
dw_add_lt_list.selectrow(0, false)
ls_lastname = sle_lookup_lastusername.Text
ls_compute_0001 = dw_add_lt_list.getitemstring(1, 'compute_0001') //ie: BATES-ALEX      [17341] {272-42201}
ls_text = f_filter_dw(ref dw_add_lt_list, ls_lastname)
dw_add_lt_list.setfilter(ls_text)
dw_add_lt_list.filter()

sle_lastname_filter.Text = ls_text
sle_filtered_count.Text = string(dw_add_lt_list.RowCount())
end event
...
long textcolor = 16711680
long backcolor = 553648127
string text = "Enter Lieutenants/Sergeants Lastname to Use:"
boolean focusrectangle = false
end type
